      ...
      {/* ── KPI Strip ── */}
        ...
        >
          {stats.map((s, i) => (
            <div
              ...
              style={i < stats.length - 1 ? { borderRight: '1px solid var(--gc-border)' } : {}}
            >
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                  s.label === 'Total' ? 'bg-[#4D8DFF]/15' :
                  s.label === 'Libres' ? 'bg-[#00D656]/15' :
                  'bg-[#EF4B5F]/15'
                }`}>
                  <span className={s.color}>{s.icon}</span>
                </div>
                <span className="text-[9px] uppercase tracking-wider font-bold text-gray-500">{s.label}</span>
              </div>
              <p className="kpi-number text-2xl text-white">{s.value}</p>
            </div>
          ))}
          {/* Ingresos Activos */}
          <div
            className="p-4 flex flex-col gap-2"
            style={{ borderLeft: '1px solid var(--gc-border)' }}
          >
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-[#00D656]/15 flex items-center justify-center">
                <span className="text-base">💰</span>
              </div>
              <span className="text-[9px] uppercase tracking-wider font-bold text-gray-500">Activo</span>
            </div>
            <p className="kpi-number text-xl text-[#00D656]">{formatCOP(ingresosActivos)}</p>
          </div>
        </div>
        ...
        {error && (
          <div
            ...
            style={{ background: 'var(--gc-surface)', border: '1px solid rgba(239,75,95,0.3)' }}
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[#EF4B5F]/10 flex items-center justify-center">
                <span className="text-[#EF4B5F] text-lg">⚠️</span>
              </div>
              <p className="text-[#EF4B5F] text-sm font-medium">{error}</p>
            </div>
          </div>
        )}
        ...
        >
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              <Gamepad2 size={16} className="text-[#00D656]" />
              Salas Gaming
            </h3>
            <div className="flex gap-2 flex-wrap">
              <button
                onClick={() => window.open('/tv', '_blank')}
                className="flex items-center gap-1.5 px-3 py-2 text-xs rounded-lg bg-white/5 border border-white/10
                  text-gray-300 hover:bg-white/10 transition-colors font-medium"
              >
                <Tv2 size={15} />
              ...
              >
                ...
                <span className="w-1.5 h-1.5 rounded-full bg-[#EF4B5F] animate-pulse" />
              </button>
              <button
              ...
              {puedeEditar && (
                <button
                  ...
                  className="btn-premium flex items-center gap-1.5 px-3 py-2 text-xs rounded-lg font-semibold"
                >
                  <Plus size={15} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#00D656]/50 transition-colors"
                style={{ background: 'var(--gc-input)', border: '1px solid var(--gc-border)' }}
              />
            </div>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 snap-x snap-mandatory">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors snap-start ${
                    tipoFiltro === filtro.value
                      ? 'bg-[#00D656] text-black'
                      : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`}
                  style={tipoFiltro !== filtro.value ? { background: 'var(--gc-input)', border: '1px solid var(--gc-border)' } : {}}
                >
                  {filtro.label}
                </button>
              ))}
