...
const STEPS = ['Información', 'Regionalización', 'Administrador', 'Revisión', 'Crear', 'Resultado'];
...
export default function PlatformProvisioningModal({ onClose, onProvision }) {
  ...
  async function next() {
    ...
    setError(null);
    try {
      const data = await onProvision({ ...form, idempotency_key: idempotencyKey });
      setResult(data);
    } catch (requestError) {
