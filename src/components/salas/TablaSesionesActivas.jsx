...
import { Clock, User, DollarSign, Pencil } from 'lucide-react';
...
export default function TablaSesionesActivas({ sesiones = [], salas = [], onAgregarTiempo, onAgregarProducto, onTrasladar, onFinalizar, onEditarAdmin }) {
  const { esAdmin } = usePermisos();
  // Sprint 0.3-C/D Fase 5: usa tick global (elimina setInterval de 60s)
  const now = useGlobalTick();
  const tiemposTranscurridos = useMemo(() => {
    const nuevosTiempos = {};
        ...
        {sesionesActivas.map((sesion) => {
          ...
          return (
            <div key={sesion.id} className="p-4 space-y-3">
              ...
              <div className={`grid gap-2 ${esAdmin ? 'grid-cols-5' : 'grid-cols-4'}`}>
                ...
                {esAdmin && (
                  <button
                    onClick={() => onEditarAdmin?.(sesion)}
                    className="h-11 flex items-center justify-center rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20 active:bg-amber-500/20 text-xs font-semibold"
                    title="Editar sesión (Admin)"
                  >
                    <Pencil size={14} />
                  </button>
                )}
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {esAdmin && (
                        <button
                          onClick={() => onEditarAdmin?.(sesion)}
                          className="px-3 py-1.5 text-xs bg-amber-500/10 text-amber-400 rounded-lg hover:bg-amber-500/20 border border-amber-500/20 hover:border-amber-500/40 hover:shadow-[0_0_10px_rgba(245,158,11,0.2)] transition-all font-semibold flex items-center gap-1"
                          title="Editar sesión (Solo Admin)"
                        >
                          <Pencil size={11} />
                          Editar
                        </button>
