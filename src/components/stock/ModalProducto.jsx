...
import { Upload, X, Calculator, TrendingUp, Image as ImageIcon, DollarSign, Percent, BarChart3, ShieldCheck } from 'lucide-react';
...
export default function ModalProducto({ abierto, producto, categorias = [], onCerrar, onGuardado }) {
  ...
  const { esAdmin, esSupervisor } = usePermisos();
  ...
  useEffect(() => {
    ...
    if (producto) {
      setForm({
        ...
        valor_total_compra: '', stock_ingreso: '',
        es_critico_arqueo: producto.es_critico_arqueo ?? false,
      });
      setImagenUrl(producto.imagen_url ?? '');
      setImagenPreview(producto.imagen_url ?? '');
    } else {
      setForm({ nombre: '', categoria: '', descripcion: '', precio: '', costo: '', stock: '', stock_minimo: '5', valor_total_compra: '', stock_ingreso: '', es_critico_arqueo: false });
      setImagenUrl('');
      setImagenPreview('');
    }
    setImagenFile(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const datos = {
        ...
        imagen_url: url || null,
        activo: true,
        es_critico_arqueo: !!form.es_critico_arqueo,
      };
  ...
  return (
    <Modal abierto={abierto} titulo={esEdicion ? 'Editar Producto' : 'Agregar Nuevo Producto'} onCerrar={onCerrar} size="lg">
      <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
        ...
        {/* ── Producto Crítico para Arqueo (solo Admin/Supervisor) ── */}
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
            <label className="flex items-center gap-3 cursor-pointer select-none">
              <div className="relative">
                <input
                  type="checkbox"
                  checked={form.es_critico_arqueo}
                  onChange={(e) => setForm(prev => ({ ...prev, es_critico_arqueo: e.target.checked }))}
                  className="sr-only peer"
                />
                <div className="w-11 h-6 rounded-full bg-white/10 border border-white/10 peer-checked:bg-[#00D656]/30 peer-checked:border-[#00D656]/50 transition-colors" />
                <div className="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-gray-400 peer-checked:bg-[#00D656] peer-checked:translate-x-5 transition-all shadow" />
              </div>
              <div className="flex items-center gap-2">
                <ShieldCheck size={16} className={form.es_critico_arqueo ? 'text-[#00D656]' : 'text-gray-500'} />
                <span className="text-sm font-medium text-white">Auditoría en Cierre</span>
              </div>
            </label>
            <p className="text-[10px] text-gray-500 mt-2 ml-14">
              Si se activa, el sistema obligará a los operadores a contar físicamente este producto al cerrar su turno.
            </p>
          </div>
        )}
