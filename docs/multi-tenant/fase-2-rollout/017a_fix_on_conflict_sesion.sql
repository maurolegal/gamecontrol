-- GAMECONTROL FASE 2B / 017a — FIX RPC agregar_productos_sesion
-- Compatibilidad con UNIQUE (tenant_id, sesion_id), sin cambios de esquema.
-- La migración 017 está reservada para platform billing modules.
...
BEGIN
  ...
  WHERE id = p_sesion_id AND tenant_id = public.current_tenant_id()
  FOR UPDATE;
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF v_idemp_existing > 0 THEN
      ...
      BEGIN
        SELECT split_part(idempotency_key, '#', 2) INTO v_hash_guardado
        FROM public.venta_items
        WHERE idempotency_key LIKE p_idempotency_key || '#%'
          AND tenant_id = public.current_tenant_id()
        LIMIT 1;
        ...
        IF v_hash_guardado = v_payload_hash THEN
          ...
          RETURN QUERY SELECT 'OK_IDEMPOTENTE'::TEXT, v_venta_id, p_sesion_id, v_idemp_existing, v_subtotal_productos,
            'batch ya procesado (idempotency_key hit, payload idéntico)'::TEXT;
          RETURN;
        ELSE
          RETURN QUERY SELECT 'ERROR_IDEMPOTENCIA_CONFLICTO'::TEXT, NULL::UUID, NULL::UUID, 0, NULL::NUMERIC,
  ...
  FROM public.ventas
  WHERE sesion_id = p_sesion_id AND tenant_id = public.current_tenant_id()
  FOR UPDATE;

  IF v_venta.id IS NOT NULL THEN
    IF v_venta.estado != 'abierta' THEN
      RETURN QUERY SELECT 'ERROR_SESION_NO_ACTIVA'::TEXT, NULL::UUID, NULL::UUID, 0, NULL::NUMERIC,
  ...
  FROM public.venta_items
  WHERE venta_id = v_venta_id AND tenant_id = public.current_tenant_id();
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    WHERE id = v_producto_id AND tenant_id = public.current_tenant_id()
    FOR UPDATE;
    ...
    SELECT * INTO v_stock_result FROM public.aplicar_movimiento_stock(
      v_producto_id, v_cantidad, 'venta', 'Venta sesión ' || p_sesion_id::TEXT,
      v_venta_id::TEXT, v_usuario_id
    );
    IF v_stock_result.status != 'OK' THEN
      RAISE EXCEPTION 'Stock falló para producto %: % - %', v_producto_id, v_stock_result.status, v_stock_result.mensaje;
    END IF;

    INSERT INTO public.venta_items (
      tenant_id, venta_id, line_no, tipo, producto_id,
      descripcion, cantidad, precio_unitario, subtotal, idempotency_key
    ) VALUES (
      public.current_tenant_id(), v_venta_id, v_line_no, 'producto', v_producto_id,
    ...
    v_item_cache := jsonb_build_object(
      'id', v_producto_id, 'producto_id', v_producto_id, 'nombre', v_producto.nombre,
      'precio', v_precio_servidor, 'cantidad', v_cantidad, 'subtotal', v_subtotal_item,
  ...
  SELECT COALESCE(SUM(subtotal), 0) INTO v_subtotal_productos
  FROM public.venta_items
  WHERE venta_id = v_venta_id AND tenant_id = public.current_tenant_id() AND tipo = 'producto';

  UPDATE public.ventas
  SET subtotal_productos = v_subtotal_productos, total = v_subtotal_productos - descuento, updated_at = NOW()
  WHERE id = v_venta_id AND tenant_id = public.current_tenant_id();

  UPDATE public.sesiones
  SET productos = v_cache_productos, total_productos = v_subtotal_productos, fecha_actualizacion = NOW()
  WHERE id = p_sesion_id AND tenant_id = public.current_tenant_id();

  RETURN QUERY SELECT 'OK'::TEXT, v_venta_id, p_sesion_id, v_items_agregados, v_subtotal_productos,
    'productos agregados. Items: ' || v_items_agregados::TEXT || ', subtotal productos: ' || v_subtotal_productos::TEXT;
END;
$function$;
