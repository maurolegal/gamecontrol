...
class GestorSalas {
    ...
    async verificarIntegridadDatos() {
        try {
            ...
            const sesionesActivas = sesionesSinDuplicados.filter(s => !s.finalizada && s.estado !== 'finalizada');
                ...
                <div class="sala-header-minimal">
                    <div class="sala-info-minimal">
                        <div class="sala-title-minimal">
                            ${sala.icono_url
                                ? `<img src="${sala.icono_url}" alt="${sala.nombre}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;vertical-align:middle;">`
    ...
    async actualizarMovimientoHoy() {
        try {
            ...
            const estadoBadge = (s) => {
                ...
                const cls = est === 'finalizada' ? 'bg-success' : (est === 'cancelada' ? 'bg-danger' : 'bg-primary');
    ...
    actualizarTemporizadores() {
        // Filtro robusto: excluir sesiones finalizadas o canceladas
        const sesionesActivas = this.sesiones.filter(s => {
            const esFinalizada = s.finalizada === true;
            const estadoFinalizado = s.estado === 'finalizada' || s.estado === 'cancelada';
    ...
    finalizarSesion(sesionId) {
        ...
        if (sesion.finalizada === true || sesion.estado === 'finalizada') {
    ...
    mostrarModalEditarSala(salaId) {
        ...
        // Ícono
        const iconoUrlInput = document.getElementById('editarIconoUrl');
        const iconoImg = document.getElementById('editarIconoImg');
        const iconoPlaceholder = document.getElementById('editarIconoPlaceholder');
        const iconoDisplay = document.getElementById('editarIconoUrlDisplay');
        const iconoFile = document.getElementById('editarIconoFile');
        if (iconoUrlInput) iconoUrlInput.value = sala.icono_url || '';
        if (iconoFile) iconoFile.value = '';
        if (sala.icono_url && iconoImg) {
            iconoImg.src = sala.icono_url;
            iconoImg.classList.remove('d-none');
            if (iconoPlaceholder) iconoPlaceholder.classList.add('d-none');
            if (iconoDisplay) iconoDisplay.classList.remove('d-none');
        } else {
            if (iconoImg) { iconoImg.src = ''; iconoImg.classList.add('d-none'); }
            if (iconoPlaceholder) iconoPlaceholder.classList.remove('d-none');
            if (iconoDisplay) iconoDisplay.classList.add('d-none');
        }
    ...
    async guardarEdicionSala(formData) {
        try {
            ...
            // Subir ícono a Cloudinary si se seleccionó un archivo nuevo
            const iconoFile = document.getElementById('editarIconoFile');
            if (iconoFile && iconoFile.files && iconoFile.files[0]) {
                const urlSubida = await this.subirIconoCloudinary(iconoFile.files[0]);
                if (urlSubida) sala.icono_url = urlSubida;
            } else {
                // Conservar url existente del campo oculto
                const urlExistente = formData.get('iconoUrl');
                if (urlExistente) sala.icono_url = urlExistente;
            }
    ...
    // ── Cloudinary: subir ícono de sala ──────────────────────────────
    async subirIconoCloudinary(file) {
        const cloudName = (typeof CLOUDINARY_CONFIG !== 'undefined' && CLOUDINARY_CONFIG.cloudName)
            ? CLOUDINARY_CONFIG.cloudName
            : (localStorage.getItem('cloudinary_cloud_name') || 'dftbhxwaa');
        const uploadPreset = (typeof CLOUDINARY_CONFIG !== 'undefined' && CLOUDINARY_CONFIG.uploadPreset)
            ? CLOUDINARY_CONFIG.uploadPreset
            : (localStorage.getItem('cloudinary_upload_preset') || 'gamehub');

        const progressEl = document.getElementById('editarIconoProgress');
        ...
        try {
            ...
            mostrarNotificacion('No se pudo subir el ícono a Cloudinary', 'error');
            return null;
        } finally {
    ...
    // Previsualizar ícono antes de subir
    previsualizarIcono(inputEl) {
